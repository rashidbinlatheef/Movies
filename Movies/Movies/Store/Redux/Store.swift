...
open class Store<State, Action> {
    ...
    private(set) public var state: State {
        didSet { observers.forEach { $0.handleState(self.state) } }
    }
    ...
    public init(
        ...
        queue: DispatchQueue = .init(label: "com.redux.store", qos: .userInitiated)
    ) {
        self.state = state
...
extension Store: Dispatcher {
    ...
    public func dispatchActions(actions: [Action]) {
